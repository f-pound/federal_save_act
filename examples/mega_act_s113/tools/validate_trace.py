...
# Books that must contain NO defaxiom (neutrality lint).  From pipeline.json
# "neutral_books" when present; otherwise a naming rule: libraries, core,
# generated tables/rules, invariants, proofs, checks and audits are neutral;
# *_text_rules, facts, scenario, hinge_* and *_model books may carry axioms.
import json as _json
_cfg_path = Path(os.environ.get("PROJECT_ROOT", ".")) / "pipeline.json"
_cfg = _json.loads(_cfg_path.read_text()) if _cfg_path.exists() else {}
NEUTRAL_PATTERNS = (r"^lib/", r"_core\.lisp$", r"_rules\.lisp$", r"_table\.lisp$", r"_invariants\.lisp$",
                    r"_proofs\.lisp$", r"_check\.lisp$", r"_audit\.lisp$", r"_instantiation\.lisp$",
                    r"_consistency\.lisp$", r"_independence\.lisp$", r"_existentials\.lisp$", r"_hinge_common\.lisp$")
def is_neutral_book(rel):
    if "neutral_books" in _cfg:
        return rel in _cfg["neutral_books"]
    if rel.endswith("_text_rules.lisp"):
        return False
    return any(re.search(p, rel) for p in NEUTRAL_PATTERNS)
...
def main():
    ...
    all_books = [f.relative_to(model_dir).as_posix() for f in list(model_dir.glob("*.lisp")) + list(model_dir.glob("lib/*.lisp"))]
    neutral_present = [b for b in all_books if is_neutral_book(b)]
    print(f"Neutrality lint: {len(neutral_present)} neutral books checked, {sum(1 for b in all_axioms.values() if is_neutral_book(b))} violations")
    deciders = {}
    for row in trace_rows:
