...
- **Try it**: two cartoon scenes — the registration counter (§ 3(b) documents) and election day at the polls (§ 303A photo ID, 3-day cure) — show the outcome, who decides each step, and what it means under your premises
...
| **lib/lsm** | **25** | **generic labeled state machine invariants (incl. gated exit), hint-free** | ✅ All Q.E.D. |
| Core | 7 | shared conflict pivots (registration, removal, voting) | ✅ All Q.E.D. |
...
| **Consistency audit (generated)** | **434** | **every party axiom + constraint proved in a concrete toy world, plus a kernel-checked independence certificate per axiom** | ✅ All Q.E.D. |
| **Burden tiers** | **7** | **Anderson–Burdick standard of review as a decision table** | ✅ All Q.E.D. |
...
| Challenger model | 19 | encapsulate + bridge rules + shared scenario; registration, removal and voting branches | ✅ All Q.E.D. |
| Government model | 11 | encapsulate + bridge rules + shared scenario; registration, removal and voting branches | ✅ All Q.E.D. |
...
- **citizen-c** (v6.5): A registered U.S. citizen who votes in person with no document in the § 303A(c) valid-photo-ID list and does not cure the provisional ballot within 3 days
...
- **66 defaxioms** across 7 books — see `reports/axiom_inventory.md` for the full classification. Every one carries a **decider** tag (legislature 5 · court 20 · fact-finder 4 · party-stipulation 31): the logic has no grey areas; each axiom is a choice, and the tag says whose. A CI lint guarantees the neutral books contain no axiom at all and no proof-trivialising construct; `reports/trusted_base_by_book.md` lists, per book, the axioms any of its theorems could depend on (the `#print axioms` analogue); and the generated **consistency audit** proves both parties' complete trusted bases satisfiable in concrete toy worlds. The **adversarial audit** then denies each axiom in turn: 92 of 94 are independent, none is redundant, and the only coupled pair on each side is the § 8(j)(2)(A) hinge — see [docs/AUDITS.md](docs/AUDITS.md).
