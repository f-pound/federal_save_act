...
## 11. Three ideas taken from Vero (v6.7)

Vero (Ye et al., arXiv:2608.13522) benchmarks agents on repository-scale code-and-proof synthesis in Lean 4. Its headline findings — proof text lives in shared lemma libraries, implementation freedom cuts both ways, agents grind on proofs instead of refactoring — were this project's v6 brief. Three further mechanisms transferred directly:

1. **Formal audit of the trusted base.** Vero accepts machine-checked proofs that a specification is unsatisfiable or a set of specifications is jointly inconsistent; 45% of the defects it found were "missing domain or operation condition" — the class of our § 3(b)(5) and over-quantified-bridge bugs. ACL2's `:defaxioms-okp t` certifies a book *without* checking that its axioms are satisfiable. `gen_consistency_audit.py` closes that gap: a concrete toy world per party, every stub defined, every axiom and encapsulate constraint restated and proved (107 theorems). The first run caught nothing in the model and one error in my own world — which is the point of having the prover check the world rather than trusting the author.
2. **`#print axioms`.** `print_axioms.py` computes, per book, the defaxioms in its include-book closure by decider — a sound upper bound on every theorem's trusted base, surfaced in the explorer. It turns the curated "0 axioms" badge into a computed fact.
3. **Screening for proof-trivialising constructs.** A CI lint rejects `skip-proofs`, `defttag`, `defattach` and friends anywhere in `model/`, alongside the existing neutrality lint and generated-book checks — the ACL2 counterpart of Vero's three grading layers.

## 12. Adversarial audit (v6.8)

`adversarial_audit.py` denies each axiom in turn (one- or two-point stub flips in the audit world) and checks the rest; with `--acl2` it also tries to prove each axiom from the others. The first runs reported several "coupled" bridges that turned out to be artifacts of over-broad toy worlds (a predicate true for every p lets a flip at an unrelated constant trip another bridge); tightening the worlds to the scenario domain — with the certified consistency book re-proved each time — left exactly one coupling per party: the scenario's alternative-process fact locked to its hinge reading. No axiom is redundant. The hinge the project was built around is therefore not an editorial choice but a structural property of the axiom sets, and the method is statute-independent.
